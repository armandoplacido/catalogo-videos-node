...
describe('CategorySequelizeRepository Integration Test', () => {
  ...
  test('should bulk insert categories', async () => {
    ...
    expect(JSON.stringify(someCategories)).toBe(
      JSON.stringify(someCategoriesModel),
    )
  })
  ...
  test('should return all categories', async () => {
    ...
    expect(someCategoriesModel).toHaveLength(10)
    expect(JSON.stringify(someCategoriesModel)).toBe(
      JSON.stringify(someCategories),
    )
  })
